...
from fields import ZINDEXABLE

# Model Set
class QuerySet(Set):
    def __init__(self, model_class):
        super(QuerySet, self).__init__(model_class._key['all'],
            db=redjango.connection)
    ...
    def _get_limit_and_offset(self):
        if ((self._limit is not None and self._offset is None)
            or (self._limit is None and self._offset is not None)):
