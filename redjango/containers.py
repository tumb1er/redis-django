...
class Container(object):
    ...
    def __getattribute__(self, att):
        if att in object.__getattribute__(self, 'DELEGATEABLE_METHODS'):
            print att, self.key
            v = partial(getattr(object.__getattribute__(self, 'db'), att), self.key)
            def f(*args, **kwargs):
                print args, kwargs
                val = v(*args, **kwargs)
                print val
    ...
    def db(self):
        ...
        if hasattr(self, 'db_cache') and self.db_cache:
            ...
            from redjango import connection
...
class Set(Container):
    ...
    # FIXME: Note, the elem argument to the __contains__(), remove(),
    ...
    def intersection_update(self, *others):
        ...
        self.db.sinterstore(self.key, [o.key for o in [self.key] + list(others)])
    ...
    def difference_update(self, *others):
        ...
        self.db.sdiffstore(self.key, [o.key for o in [self.key] + list(others)])
...
class TypedList(object):
    ...
    Arguments:
        key -- the Redis key this container is stored at
        target_type -- can be a Python object or a redjango model class.
    ...
    If target_type is not a redjango model class, the target_type should
    ...
    target_type also accepts a string that refers to a redjango model.
    """

    def __init__(self, key, target_type, type_args=None, type_kwargs=None, **kwargs):
        ...
        self._redjango_model = issubclass(self.klass, Model)
